...
public class ViewableCommandThread extends CommandThread {
	...
	protected void exec(HashMap<String, Object> command) throws IOException {
		...
		if (type == ANSWER) {
			// 收到回应
			CommandType answerType = CommandType.valueOf((String)command.get("answerType"));
			switch (answerType) {
				// 确认回应命令类型
				case ADD -> {
				...
				case GETPATH -> {
					if ((boolean)command.get("err")) {
						Exception exception = JSON.parseArray((String)command.get("exception"), IOException.class).get(0);
						exceptionManager.throwException(exception);
					}
					else {
						List<CustomPath> paths = JSON.parseArray((String)command.get("paths"), String.class).stream()
